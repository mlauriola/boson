    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', async () => {
            ...
            for (const id of itemsToDelete) {
                try {
                    await fetch(`/api/damaged-items/${id}`, { method: 'DELETE' });
                    successCount++;
                } catch (e) {
...
async function loadPermissions() {
    try {
        const response = await fetch('/api/damaged-items/permissions');
        if (response.ok) {
            ...
            if (!currentPermissions.canEdit) {
                ...
                // Export is allowed for viewers? Route has checkAdminOrSuper. So only edit/admin.
...
function setupEventListeners() {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            window.location.href = '/api/damaged-items/export';
        });
    }
...
async function deleteSelectedItems() {
    ...
    // We'll delete sequentially for now as API is single delete
    // A better approach would be a bulk-delete API
    let successCount = 0;
...
function openEditModal(id) {
    ...
    document.getElementById('modalTitle').textContent = 'Edit Damaged Item #' + id;
    document.getElementById('itemId').value = id;

    document.getElementById('itemCode').value = item.ItemCode || '';
    document.getElementById('description').value = item.Description || '';
    document.getElementById('status').value = item.Status || 'Damaged';

    // Using raw style manip because modal has inline styles that might override classes
    modal.style.display = 'block';
}

function openAddModal() {
    document.getElementById('modalTitle').textContent = 'Add Damaged Item';
    document.getElementById('itemId').value = '';

    // Reset form
    document.getElementById('itemCode').value = '';
    document.getElementById('description').value = '';
    document.getElementById('status').value = 'Damaged';
...
async function saveItem() {
    ...
    try {
        const url = id ? `/api/damaged-items/${id}` : '/api/damaged-items';
...
function getStatusBadge(status) {
    switch (status) {
        ...
        case 'Not Working': return 'badge-warning';
        case 'Fixed': return 'badge-success';
        case 'Repairing': return 'badge-info';
